...
Always respond as if you're personally posting/replying on social media.`,
  ...
  testIf(hasApiKey)(
    'should respond to a crypto discussion with expertise',
    async () => {
    ...
    What's everyone's thoughts on the current market cycle? Are we in for another run or is this a bull trap? 🤔"`;
      ...
      const response = await socialAgent.chat(`Respond to this crypto post: ${cryptoPost}`);
  ...
  testIf(hasApiKey)(
    'should respond to a tech startup post with relevant experience',
    async () => {
    ...
    Product-market fit is real but so is the grind. Any advice for scaling from 0 to 1? 🚀"`;
      ...
      const response = await socialAgent.chat(`Respond to this startup post: ${startupPost}`);
  ...
  testIf(hasApiKey)(
    'should maintain consistent character across multiple interactions',
    async () => {
  ...
  testIf(hasApiKey)(
    'should update context dynamically and reflect changes',
    async () => {
      ...
      const response = await socialAgent.chat(`Respond to this positive post: ${celebrationPost}`);
  ...
  testIf(hasApiKey)(
    ...
    async () => {
      ...
      const controversialPost =
        'Crypto is just a ponzi scheme and everyone knows it. Change my mind.';
...
describe.skip('Model Recommendation Integration', () => {
  ...
  testIf(hasApiKey)('should find best model for specific use case', async () => {
    ...
    if (bestCodingModel) {
      expect(
        bestCodingModel.useCase.some(
          uc => uc.toLowerCase().includes('coding') || uc.toLowerCase().includes('code')
        )
      ).toBe(true);
    }
  });
